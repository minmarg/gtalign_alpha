...
#include "dpagc_tfm_ssw_btck_specscore.cuh"

// #define CUDPAGC_TFM_2DSPECAN_SS_BTCK_TESTPRINT 0 //3024//4899
...
// -------------------------------------------------------------------------
// ExecDPAGCTFMSSwBtckSpecScores3264x: execute dynamic programming with 
// affine gap cost model, secondary structure and backtracking information 
// using shared memory and 32(64)-fold unrolling along the diagonal of 
// dimension CUDP_2DSPECAN_DIM_D;
// NOTE: memory pointers should be aligned!
// specscores, address of spectral scores between a query and reference;
// wrkmemtm, address of transformation matrices for query-reference pairs;
    ...
    char* __restrict__ btckdata)
{
    ...
    //query and reference coordinates + transformation matrix:
    //__shared__ float qryCoords[pmv2DNoElems * CUDP_2DSPECAN_DIM_D];
    float qry2DX, qry2DY, qry2DZ;
    __shared__ float rfnCoords[pmv2DNoElems * CUDP_2DSPECAN_DIM_DpX];
    __shared__ float tfmCache[nTTranformMatrix];
    char qrySS;//query secondary structure assignment; reference below:
    __shared__ char rfnSS[CUDP_2DSPECAN_DIM_DpX+1];//+1 to avoid bank conflicts
    ...
    if(0 <= (x+CUDP_2DSPECAN_DIM_D) && (x+CUDP_2DSPECAN_DIM_D) < dbstrlen)
        //NOTE: blockDim.x==CUDP_2DSPECAN_DIM_D
        DPLocCacheRfnCoords<CUDP_2DSPECAN_DIM_D>(rfnCoords, dbpos + CUDP_2DSPECAN_DIM_D);
    else DPLocInitCoords<CUDP_2DSPECAN_DIM_D/*shift*/,CUDP_DEFCOORD_RFN>(rfnCoords);
    ...
    if(USESS) {
        ...
        if(0 <= (x+CUDP_2DSPECAN_DIM_D) && (x+CUDP_2DSPECAN_DIM_D) < dbstrlen)
            //NOTE: blockDim.x==CUDP_2DSPECAN_DIM_D
            DPLocCacheRfnSS<CUDP_2DSPECAN_DIM_D>(rfnSS, dbpos + CUDP_2DSPECAN_DIM_D);
        else DPLocInitSS<CUDP_2DSPECAN_DIM_D/*shift*/,0/*VALUE*/>(rfnSS);
    }
    ...
    //READ TRANSFORMATION MATRIX for query-reference structure pair
    if(threadIdx.x < nTTranformMatrix) {
//         //best transformation matrix written at position stepnumber=0;
//         uint mloc0 = ((blockIdx.z/*qryndx*/ * maxnsteps + 0/*stepnumber*/) * ndbCstrs +
//             blockIdx.y/*dbstrndx*/) * nTTranformMatrix;
//         tfmCache[threadIdx.x] = wrkmemtmibest[mloc0 + threadIdx.x];
        uint tfmloc = (blockIdx.z/*qryndx*/ * ndbCstrs + blockIdx.y/*dbstrndx*/) *
            nTTranformMatrix;
        tfmCache[threadIdx.x] = tfmmem[tfmloc + threadIdx.x];
    }
...
// #endif
    ...
    float d02 = GetD02_dpscan(qrylen, dbstrlen);
    ...
    for(int i = 0; i < ilim/*CUDP_2DSPECAN_DIM_X*/; i++)
    {
        ...
        if(qry2DX < CUDP_DEFCOORD_QRY_cmp && CUDP_DEFCOORD_RFN_cmp < rfn2DX) {
            ...
            //    rfnCoords[GetCoordsNdx(pmv2DZ, threadIdx.x + i)]
            //);
            val1 += GetPairScore(d02, val2);//superposition score
        }
...
#ifdef CUDPAGC_TFM_2DSPECAN_SS_BTCK_TESTPRINT
        if(blockIdx.y==CUDPAGC_TFM_2DSPECAN_SS_BTCK_TESTPRINT){
            printf(" d=%u(%u) s=%u i%02d/%u (t%02u): len= %d addr= %u SC= %.4f (yx: %d,%d) "
                    "MM= %.6f  MG= %.6f  GM= %.6f  BTCK= %d  "// MAX= %.6f COORD= %x\n"
...
// Instantiations
//
#define INSTANTIATE_ExecDPAGCTFMSSwBtckSpecScores3264x(tpUSESS) \
    template \
    __global__ void ExecDPAGCTFMSSwBtckSpecScores3264x<tpUSESS>( \
        const uint blkdiagnum, \
...
INSTANTIATE_ExecDPAGCTFMSSwBtckSpecScores3264x(false);
INSTANTIATE_ExecDPAGCTFMSSwBtckSpecScores3264x(true);
