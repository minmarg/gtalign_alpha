...
#include "dpssw_btck_specscore.cuh"

// #define CUDP_2DSPECAN_SS_BTCK_TESTPRINT 0 //3024//4899
...
// ssweight, weight for secondary structure scores;
...
// -------------------------------------------------------------------------
// ExecDPSSwBtckSpecScores3264x: execute dynamic programming with secondary 
// structure and backtracking information using shared memory and 
// 32(64)-fold unrolling along the diagonal of dimension 
// CUDP_2DSPECAN_DIM_D;
...
// #endif
    ...
    for(int i = 0; i < ilim/*CUDP_2DSPECAN_DIM_X*/; i++)
    {
        ...
        val1 += (float)(qrySS == rfnSS[threadIdx.x+i]) * ssweight;
...
#ifdef CUDP_2DSPECAN_SS_BTCK_TESTPRINT
        if(blockIdx.y==CUDP_2DSPECAN_SS_BTCK_TESTPRINT){
...
// Instantiations
//
#define INSTANTIATE_ExecDPSSwBtckSpecScores3264x(tpGAP0,SCORES_TYPE) \
    template \
    __global__ void ExecDPSSwBtckSpecScores3264x<tpGAP0,SCORES_TYPE>( \
        const uint blkdiagnum, \
...
INSTANTIATE_ExecDPSSwBtckSpecScores3264x(false, CUSA_DP_SCORES_PSD);
INSTANTIATE_ExecDPSSwBtckSpecScores3264x(false, CUSA_DP_SCORES_SPT);
INSTANTIATE_ExecDPSSwBtckSpecScores3264x(true, CUSA_DP_SCORES_PSD);
INSTANTIATE_ExecDPSSwBtckSpecScores3264x(true, CUSA_DP_SCORES_SPT);
